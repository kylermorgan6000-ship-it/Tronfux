...
import { creditBrokerUser, deductBrokerUser } from '@/lib/api' // use centralized api functions
...
const handleCredit = async () => {
  ...
  message.value = ''
  try {
    const res = await creditBrokerUser(email.value, amount.value, currency.value)
    message.value = res.message || 'Successfully credited user'
  } catch (err: any) {
    message.value = err.response?.data?.message || err.message || 'Error'
  } finally {
    loading.value = false
  }
}
...
const handleDeduct = async () => {
  ...
  message.value = ''
  try {
    const res = await deductBrokerUser(email.value, amount.value, currency.value)
    message.value = res.message || 'Successfully deducted from user'
  } catch (err: any) {
    message.value = err.response?.data?.message || err.message || 'Error'
  } finally {
    loading.value = false
...
<template>
  <AdminLayout>
    <section class="p-3 min-h-screen text-white space-y-4">
      <h1 class="text-xl font-bold flex items-center gap-2"><User class="w-5 h-5 text-green-400" /> Credit / Deduct User (Broker)</h1>
