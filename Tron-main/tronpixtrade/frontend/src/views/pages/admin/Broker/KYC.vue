...
import {
  User, CheckCircle, XCircle, Loader2, Search, Eye, Trash
} from 'lucide-vue-next'
import { fetchBrokerKycUsers, approveBrokerKyc, rejectBrokerKyc, deleteBrokerKyc } from '@/lib/api'
...
const loadKyc = async () => {
  ...
  try {
    ...
    kycList.value = users.map((u) => ({
      ...
      documentUrl: u.documentUrl ?? `https://via.placeholder.com/300x200.png?text=${encodeURIComponent('Document')}`,
    }))
  } catch (e) {
...
onMounted(() => { void loadKyc() })

// ------------------------- COMPUTED -------------------------
const filteredList = computed(() =>
  kycList.value.filter(u =>
...
const approveKYC = async () => {
  ...
  try {
    ...
    console.warn('Approve KYC endpoint failed, updating locally', e)
    selectedUser.value.status = 'Approved'
  } finally {
...
const rejectKYC = async () => {
  ...
  try {
    ...
    console.warn('Reject KYC endpoint failed, updating locally', e)
    selectedUser.value.status = 'Rejected'
  } finally {
    actionLoading.value = false
    showModal.value = false
