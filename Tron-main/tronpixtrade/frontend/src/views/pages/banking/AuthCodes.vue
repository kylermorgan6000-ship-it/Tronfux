...
import { fetchSecuritySteps, verifySecurityStep } from '@/lib/api'
...
const currentStepData = computed(
  () => securitySteps.value[step.value] || { name: '', explanation: '', code: '', account: '' }
)
const otpString = computed(() => otp.value.join(''))
const canContinue = computed(
  () => otpString.value.length === CODE_LENGTH && !otp.value.includes('')
)
...
const fetchSteps = async () => {
  loading.value = true
  try {
    const data = await fetchSecuritySteps()
    securitySteps.value = data
  } catch (err) {
...
const verifyCode = async () => {
  ...
  try {
    ...
    await verifySecurityStep(currentStep.id, otpString.value)
